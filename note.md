...
- 渦巻き型から派生して、こんなことを考えた
    - 1種類のコンピュータをすべて接続できれば、それだけで 4950点を獲得できるので、
    焼きなましを繰り返してもたかだか1000点くらいしかいかないなら、
    1種類だけに専念したほうがスコア伸びそう。
    - 約5000点を安定して取れれば、それだけで、250k を達成できる。
    - E字型にすれば、まだ実装できそうだし、隙間でそれぞれ繋がる可能性があるから、
    4950 + α　が見込めそう。
    - だが、実装する時間がないなあ。
- DFS をやるとしたら、条件が複雑にならないように「接続されていないコンピュータ」だけを
    動かすことにする
    - 打ち切りの条件は、以下の2つのどちらか。
        - 自分と同種のコンピュータを接続しているケーブル上に移動する
        - 自分と同種のコンピュータが、4方向のどこかに見える。
            - これは、 can_connect メソッドが使える
    - 逆に、深さの上限まで探索して見つからなければ、元いた場所に戻す。
- 格子状に配置して、1種類だけ全部つなげるのを狙う。
    - 種類1 は、格子に乗っていれば正の点数
    - 傾斜をかけることで、「詰まる」のは防げそう
    - 余計なコンピュータが格子点に入って囲まれてしまったら大変そうだが、
        数カ所詰まったところで大した影響はないと思われる。
    - 実装してみたが、うまくいかず。
- 終了後
    - ランダムに移動して都度評価する部分の移動回数を1回にしたら、かなり点数が跳ねた。
    - よく考えたら納得。一気に登り切るよりも少し登って評価してを繰り返した方が良くなるよなあ。
    - これは、考察の甘さだった。
